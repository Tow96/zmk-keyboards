...
# Generate/update .gitignore from config/west.yml
update_gitignore() {
...
init() {
  ...
  eval "$COMMAND west init -l config"
  update
...
build_target() {
  ...
  while IFS='|' read -r board shield snippet cmake_args artifact_name; do
    if [ "$artifact_name" = "$target_name" ]; then
      ...
      # Execute build
      eval "$COMMAND west build \"\${build_args[@]}\" -- \"\${cmake_args_array[@]}\"" || return 1
...
# Build all targets in build YAML file
build_all() {
  local strat_time
  start_time=$(date +%s)
...
if ! command -v "$RUNTIME" &>/dev/null; then
    log_error "$RUNTIME could not be found. Please install to proceed."
    exit 1
fi
...
list)
  ...
  ;;
build)
  # no param given: build all
  if [ -z "${2:-}" ]; then
    build_all
    exit 0
  fi

  # If no wildcard is given: build exact target
  if [[ "${2}" != *[\*\?\[]* ]]; then
      build_target "${2}"
  fi
